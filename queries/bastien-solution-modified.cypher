MATCH (s1:Service)-[:HAS]->(p1:Path)-[:HAS]->(arrival:Stop)-[:CALLS_AT]->(:Station)
  <-[:CALLS_AT]-(departure:Stop)<-[:HAS]-(p2:Path)<-[:HAS]-(s2:Service)
WHERE arrival.arrives IS NOT NULL AND s1 <> s2
WITH arrival,
    departure, 
...
MATCH (:Station {name: "LYDNEY"})<-[:CALLS_AT]-(d:Stop)
     (()<-[:HAS]-(p1:Path)-[:HAS]->(x:Stop)-[:CONNECTS_TO|NEXT]->(y:Stop)){0,2}
      (a:Stop)-[:CALLS_AT]->(:Station {name: "CHURCH STRETTON"})
RETURN [p1] + pn AS paths, 
  duration.between(d.departs, a.arrives) AS total_duration
